    ...
    <div ref="visualization">
      ...
      <button @click="centralMessages4(1)" class="central-button centralshow4">Central to Normal</button> -->
            ...
            <div ref="messages1" class="message-container1" v-if="(message1 || message5) && showMessages1">
                <span v-for="msg in messages1_text" :key="msg.id" :class="getMessageClass(msg.speakerid)">{{ msg.text }}
                </span>
            </div>
            <div ref="messages2" class="message-container2" v-if="(message2 || message6) && showMessages2">
                <span v-for="msg in messages2_text" :key="msg.id" :class="getMessageClass(msg.speakerid)">{{ msg.text }}
                </span>
            </div>
            <div ref="messages3" class="message-container3" v-if="(message3 || message4) && showMessages3">
                <span v-for="msg in messages3_text" :key="msg.id" :class="getMessageClass(msg.speakerid)">{{ msg.text }}
                </span>
            </div>
            <div ref="messages4" class="message-container4" v-if="(message4 || message8) && showMessages4">
                <span v-for="msg in messages4_text" :key="msg.id" :class="getMessageClass(msg.speakerid)">{{ msg.text }}
                </span>
            </div>
            <!-- warehouse messages -->
